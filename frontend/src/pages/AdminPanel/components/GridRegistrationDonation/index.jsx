import {
	formatUkrainianDateBirthday,
	formatUkrainianDateTimeDonation,
} from "../../../../helpers/date";
import { apiClient } from "../../../../apiClient";
function GridRegistrarionDonation({
	registrationDonation,
	type,
	setRegistrationDonation,
}) {
	const handleToggle = async (itemId) => {
		setRegistrationDonation((prevState) => {
		...
		try {
			await apiClient.put("registration-donation", { id: itemId });
		} catch (error) {}
	};
	const handleDelete = async (itemId) => {
		...
		try {
			await apiClient.delete(`registration-donation/${itemId}`);
		} catch (error) {}
	};
	...
	return (
		<div className='w-11/12 mx-auto flex flex-col'>
			...
			<div className='grid-registration w-full border-y-[1px]'>
				<span className='border-x-[1px] p-2'>ПІБ</span>
				<span className='p-2'>Номер телефону</span>
				<span className='border-x-[1px] p-2'>Дата народження</span>
				<span className='p-2'>Дата та час донації</span>
				<div className='border-x-[1px] p-2'></div>
			</div>

			<div className='flex flex-col w-full'>
				{registrationDonation?.map((item) => (
					<div className='grid-registration w-full border-y-[1px]'>
						<span className='border-x-[1px] p-2'>
							{item.fullName}
						</span>
						<span className='p-2'>{item.phone}</span>
						<span className='border-x-[1px] p-2'>
							{formatUkrainianDateBirthday(item.birthday)}
						</span>
						<span className='p-2'>
							{formatUkrainianDateTimeDonation(item.donationDate)}
						</span>
...
export default GridRegistrarionDonation;
